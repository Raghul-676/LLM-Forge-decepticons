...
# ):
#     """
#     Deterministic rescue plan for common private-loan/debt scenarios.
...
#     fallback = [
#         {
#             "label": "Deterministic loan-contract research fallback",
#             "why_research_it": (
#                 "Fallback authority targeting for a private loan dispute."
#             ),
#             "authority_targets": [
#                 {
...
#         3000
#     )
#     source_text = build_legal_reranker_input(candidates)
...
# Score each source:
# 3 = DIRECT: the provided excerpt itself states a rule/holding directly
#     applicable to the user's actual transaction.
# 2 = LIMITED BUT USEFUL: the excerpt states a relevant rule, but only with
#     a narrower scope or factual precondition.
# 1 = BACKGROUND: related vocabulary, statute discussion, analogy, or a
#     materially different transaction/regime.
# 0 = IRRELEVANT.

# For every score 2 or 3, you MUST provide at least one safe_proposition.
# Each safe_proposition must include:
# - proposition: the narrow legal proposition the excerpt actually supports;
# - quote_anchor: an EXACT contiguous phrase copied from the supplied excerpt
#   (prefer 4-14 words) that anchors that proposition;
# - scope_limit: the factual or doctrinal limit that prevents overgeneralizing.

# STRICT ENTAILMENT RULES:
# - A case mentioning a loan does NOT establish a general right to a remedy.
# - A case about executing a mortgage/security for a loan does NOT establish
#   specific performance of a simple repayment promise.
...
# - Special bank/SARFAESI/recovery regimes do not apply to a private friend
#   loan without matching facts.
# - A historical/replaced statute cannot support a present-day proposition
#   as current law by itself.
# - Do not repair missing propositions using model knowledge.
# - If the passage does not contain an exact quote anchor for the proposed
#   proposition, score it at most 1.
...
#         {
#           "proposition": "...",
#           "quote_anchor": "exact words from excerpt",
#           "scope_limit": "..."
#         }
#       ]
#     }
...
# Evaluate every supplied source.
# """
...
# Evaluate passage-level applicability and return only propositions directly
# anchored in the supplied excerpt.
# """
...
#             {"role": "user", "content": user_prompt}
#         ],
#         max_completion_tokens=LEGAL_RERANK_MAX_TOKENS,
#         required_keys=["results"],
#         label="Legal source verifier"
#     )
...
#         source_id = normalize_text(result.get("source_id", "")).upper()
#         candidate = candidate_map.get(source_id)
#         if not candidate:
#             continue

#         try:
#             score = max(0, min(int(result.get("score", 0)), 3))
#         except Exception:
#             score = 0

#         valid_props = []
#         source_norm = normalize_match_text(candidate.get("text", ""))

#         for proposition in result.get("safe_propositions", []) or []:
#             if not isinstance(proposition, dict):
#                 continue

#             prop_text = normalize_text(proposition.get("proposition", ""))
#             anchor = normalize_text(proposition.get("quote_anchor", ""))
#             scope = normalize_text(proposition.get("scope_limit", ""))
#             anchor_norm = normalize_match_text(anchor)

#             # Deterministic anchor enforcement: the model cannot create an
#             # allowed proposition unless its exact anchor is actually present
...
#         # A 2/3 score without any passage-anchored proposition is unsafe.
#         if score >= 1 and not valid_props:
#             score = 1

#         score_map[source_id] = {
#             "score": score,
#             "reason": normalize_text(result.get("reason", "")),
#             "safe_propositions": valid_props
#         }
...
#             {
#                 "score": 0,
#                 "reason": "No passage-scoped verifier result returned.",
#                 "safe_propositions": []
#             }
#         )
...
#         item["legal_score"] = evaluation["score"]
#         item["legal_reason"] = evaluation["reason"]
#         item["safe_propositions"] = evaluation["safe_propositions"]

#         current_status = item.get("current_law_status", {})
#         if current_status.get("is_replaced", False) and item["legal_score"] > 1:
#             item["legal_score"] = 1
#             item["safe_propositions"] = []
#             item["legal_reason"] = (
#                 item["legal_reason"]
#                 + " Historical-source clamp: this Act has been replaced for "
#                 + "present-day law by "
#                 + current_status.get("successor_title", "a successor statute")
#                 + ", so it cannot independently support a current-law claim."
#             )
...
#         key=lambda item: (
#             item["legal_score"],
#             len(item.get("safe_propositions", [])),
#             item["retrieval_score"]
#         ),
#         reverse=True
#     )
...
#         if item["legal_score"] >= 2
#         and item.get("safe_propositions")
#     ][:FINAL_SOURCE_K]

#     next_question = normalize_text(data.get("next_question", ""))
#     normalized_data = {
#         "results": raw_results,
#         "needs_more_information": bool(data.get("needs_more_information", False))
#         and bool(next_question),
#         "next_question": next_question
#     }

#     return reranked, selected, normalized_data
...
# 10. Add one sentence saying that issues not covered by the allowed
#     claims remain unverified by the current retrieval.
# 11. End with one short note that the research corpus may contain
...
#         "710K Corpus → Authority Verification → Claim Audit → Answer"
#     )
...
For every score 2 or 3, provide at least one safe_proposition:
- proposition: the narrow legal proposition the excerpt actually supports;
...
"""
    ...
    for result in raw_results:
        ...
        for proposition in result.get(
            ...
            resolved_anchor = resolve_passage_quote_anchor(
        ...
        # Keep the legal standard strict: score 2/3 still needs a proposition
        # genuinely anchored in the retrieved passage. The difference from the
        # old version is that harmless quote-copy variation can now be repaired
        # by finding the closest ACTUAL clause from the source.
        if score >= 2 and not valid_props:
            score = 1
    ...
    for number, candidate in enumerate(
        ...
        evaluation = score_map.get(
            source_id,
            {
                "score": 0,
                "reason": (
                    "No passage-scoped verifier result returned."
                ),
                "safe_propositions": []
            }
        )
        ...
        item["legal_score"] = evaluation["score"]
        item["legal_reason"] = evaluation["reason"]
        item["safe_propositions"] = evaluation[
            "safe_propositions"
        ]
